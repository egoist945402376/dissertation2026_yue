...
def two_moons_data(num_samples, seed=None):
    """Sample two noiseless curved components within [-1, 1]^2."""
    ...
    # Centre and rescale the classical two-moons geometry to [-1, 1]^2.
    X = np.stack((
        (x - 0.5) / 1.5,
        (y - 0.25) / 0.75,
    ), axis=1)
...
if __name__ == '__main__':
    ...
    torch.save(diff_model.backward_process.state_dict(), 'two_moons_no_noise_backward_process_2.pt')
