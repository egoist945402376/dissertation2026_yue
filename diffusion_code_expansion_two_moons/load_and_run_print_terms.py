...
if __name__ == '__main__':
    ...
    bp.load_state_dict(torch.load('two_moons_backward_process.pt', map_location='cpu'))
    ...
    for label, lamda in lambdas.items():
        ...
        rows.append({'dataset': 'two_moons', 'lambda_label': label,
                     'lambda': lamda, **t})
    ...
    with open('two_moons_bound_terms.csv', 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)
    print('\nWritten to two_moons_bound_terms.csv')
